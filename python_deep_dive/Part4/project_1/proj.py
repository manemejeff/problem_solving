...
class TestAccount(unittest.TestCase):
    ...
    def test_create_account_blank_first_name(self):
        ...
        with self.assertRaises(ValueError):
            a = Account(self.account_number, '', self.last_name, self.tz, self.balance)

    def test_create_account_negative_balance(self):

        with self.assertRaises(ValueError):
            a = Account(self.account_number, self.first_name, self.last_name, self.tz, -100)

    def test_accountwithdraw_ok(self):
        a = Account(self.account_number, self.first_name, self.last_name, self.tz, self.balance)
        withdrawal_ammount = 20
        conf_code = a.withdraw(withdrawal_ammount)
        self.assertTrue(conf_code.startswith('W-'))
        self.assertEqual(self.balance - withdrawal_ammount, a.balance)

if __name__ == '__main__':
    # a = Account('A100', 'Illidan', 'Stormrage', timezone=TimeZone('MST', -7, 0), initial_balance=100)
    # print(a.balance)
